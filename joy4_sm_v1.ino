...
//    subsequently I discovered this was caused by taking the 5v feed from RAW as per the wiring diagram
//    taking the 5v feed from VCC brought the values back into the range.
//    Remember we can often "fix it in software", but sometimes shouldn't :)  
// 3) Added additional debug level of 6 which only outputs on trigger.
// 4) Swapped the parameter orderin the call to send_command 

// SM 24 Jun 24: Fixed inverted Z-Rotation, bumped up Z-Rotation Speed
...
// 6: Output as per 5, but only when input is detected.
int debug = 6;
...
int DEADZONE = 10; // Recommended to have this as small as possible for V2 to allow smaller knob range of motion.
...
int centerPoints[8];
float centerPositiveScaling[8];
float centerNegativeScaling[8];
...
// Function to calculate positive and negative scaling should the centerpoints be significantly away from 512.
// This is gives two simple linear gradients per input (i.e. a scaling factor)
// Scaling prevents imbalance across the opposing sets of joysticks, which led to an unintended induced roatation.
void calcScalings()
{
  for(int i=0; i<8; i++)
  {
      centerPositiveScaling[i] = 512.0 / (1023.0-centerPoints[i]);
      centerNegativeScaling[i] = 512.0 / centerPoints[i];
  } 
...
void loop() {
  ...
  for(int i=0; i<8; i++)
  {
    ...
    // SM: Added scaling for offset centerPoints
    if(rawReads[i] > centerPoints[i])
    {
      centered[i] = (rawReads[i] - centerPoints[i]) * centerPositiveScaling[i];
    }
    else
    {
      centered[i] = (rawReads[i] - centerPoints[i]) * centerNegativeScaling[i];
    }
  ...
  if((abs(centered[AY])>DEADZONE)&&(abs(centered[BY])>DEADZONE)&&(abs(centered[CY])>DEADZONE)&&(abs(centered[DY])>DEADZONE)){
    rotZ = -(+centered[AY] +centered[BY] +centered[CY] +centered[DY])/1;
    rotX = 0;
    rotY = 0;
  } else {
    rotZ = 0;
...
// Report debug 4 and 5 info side by side for direct reference if enabled. Very useful if you need to alter which inputs are used in the arithmatic above.
  ...
  // Check if we actually have any user input.
  // No need to end anything otherwise, and it may lead to performance issues.
  if(centered[0]!=0 || centered[1]!=0 || centered[2]!=0 || centered[3]!=0 ||
     centered[4]!=0 || centered[5]!=0 || centered[6]!=0 || centered[7]!=0 ) 
  {
    if(debug == 6){
        Serial.print("Movement ! AX:");
        Serial.print(centered[0]);
        Serial.print(",");
        Serial.print("AY:");
        Serial.print(centered[1]);
    ...
    // Parameters changed by SM to behave in the same way as the 3DConnexion default in the training app.
